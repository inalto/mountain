...
class ReportImport
{
    ...
    public static function import($truncate=false,$skip=false,$start_from)
    {
        $reports = DB::connection('mysqlold')->table('node')->where('type', 'relazioni')->where('status', 1)
            ...
            ->leftJoin('field_data_field_tempo_di_salita', function ($q) {
                $q->on('field_data_field_tempo_di_salita.entity_id', '=', 'node.nid');
            })
            ->leftJoin('field_data_field_tempo_di_discesa', function ($q) {
                $q->on('field_data_field_tempo_di_discesa.entity_id', '=', 'node.nid');
            })
            ->leftJoin('field_data_field_lunghezza', function ($q) {
                $q->on('field_data_field_lunghezza.entity_id', '=', 'node.nid');
            })
            ->leftJoin('field_data_field_quota_di_partenza', function ($q) {
                $q->on('field_data_field_quota_di_partenza.entity_id', '=', 'node.nid');
            })
            ->leftJoin('field_data_field_altitudine', function ($q) {
                $q->on('field_data_field_altitudine.entity_id', '=', 'node.nid');
            })
            ->leftJoin('field_data_field_dislivello', function ($q) {
                $q->on('field_data_field_dislivello.entity_id', '=', 'node.nid');
            })
            ->where('node.nid','>=',$start_from)
            ->orderBy('node.nid')
            ->get();
        ...
        if ($truncate) {
            ...
            Report::all()->each->delete();
            DB::table('reports')->truncate();
            DB::table('media')->where('model_type','App\Models\Report')->delete();
            DB::table('reports_translations')->truncate();
        ...
        foreach ($reports as $key => $value) {
            ...
            if ($skip) {
                if (Report::where('nid',$value->nid)->get()->first())
                {
                    continue;
                }
            }
            ...
            $r->difficulty   = self::resolveDifficulty(intval($value->field_difficolt__value));
