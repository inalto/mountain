<section class="py-3 body-font">
    <div class="px-1 py-1 mx-auto">
        <div class="flex flex-wrap">
            @forelse ($havebeentheres as $havebeenthere)
            <div class="w-full p-1 md:w-1/2 lg:w-1/3 xl:w-1/4 ">
                <div class="flex flex-col h-full overflow-hidden bg-gray-50 dark:bg-gray-850 rounded-lg shadow-lg">
                    <div class="flex-grow-0">
                        @if ($havebeenthere->getFirstMediaUrl('havebeenthere_photos'))
                        <a class="block border-b-4 transition duration-500 border-white dark:border-black focus:outline-none hover:border-inalto-400 hover:dark:border-inalto-700 focus:border-inalto-400 dark:focus:border-inalto-700" href="#">
                            <img class="object-cover object-center w-full h-48" src="{{ $havebeenthere->getFirstMediaUrl('havebeenthere_photos') }}" alt="" /></a>
                        @else
                        <a class="block border-b-4 transition duration-500 border-white dark:border-black focus:outline-none hover:border-inalto-400 hover:dark:border-inalto-700 focus:border-inalto-400 dark:focus:border-inalto-700" href="#">
                    ...
                    <div class="flex flex-col flex-grow p-3 bg-white dark:bg-gray-800">
                        <x-avatar :user="$havebeenthere->owner" show="true"></x-avatar>
                        <h2 class="flex-grow-0  mb-3 text-lg font-medium text-gray-900 dark:text-gray-100 title-font font-bold" title="{{$havebeenthere->nid}}" data-noi-ndex>
                            {{ $havebeenthere->title }}
                        </h2>
                        <div class="flex-grow leading-relaxed mb-4 text-sm text-gray-800 dark:text-gray-300" data-no-index>
                            {{Str::of(strip_tags($havebeenthere->excerpt))->words(20,'...') }}
                        </div>
                        <div class="flex justify-between flex-grow-0">
                            <div>
                                ...
                                <span class="inline-flex items-center text-sm leading-none text-gray-600 dark:text-gray-300">
                                    ...
                                    {{ $havebeenthere->drop_p  }} m
                            </div>
                            <div>
                                <a href="{{ route('report.show',$havebeenthere->report?->path)}}" tabindex="-1" class="py-2 px-3 border text-gray-600 hover:bg-gray-50 dark:text-gray-300 text-sm rounded-lg dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-900 inline-flex items-center dark:hover:text-inalto-500 hover:text-inalto-400 md:mb-2 lg:mb-0">{{ __('inalto.read')}}
            ...
            @empty
            <p>No havebeenthere</p>
            @endforelse

            {{ $havebeentheres->links() }}
        </div>
    </div>
</section>