<form wire:submit.prevent="submit" class="p-3">
    ...
    <div class="form-group {{ $errors->has('user.password') ? 'invalid' : '' }}">
        <x-jet-label class="form-label required" for="password">{{ trans('cruds.user.fields.password') }}</x-jet-label>
        <x-jet-input class="form-control" type="password" name="password" id="password" required wire:model.defer="password">
        <div class="validation-message">
